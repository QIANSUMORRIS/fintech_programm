...
#先用logistic进行训练和预测
df = pd.read_csv('./features_001.csv')
# df = df.iloc[:,1:]
# print(df.head(10))
# df.drop(['recoveries','collection_recovery_fee', 'total_rec_prncp'], axis = 1, inplace =True)#
#df.drop(['recoveries','collection_recovery_fee'], 1, inplace = True)
# print(df.head(5))
Y = df.loan_status
...
train_pre = metrics.precision_score(train_predict, y_train)
...
print("在训练集上的精确率值为%.4f"% train_pre, end=' ')
...
test_pre = metrics.precision_score(test_predict, y_test)
test_rec = metrics.recall_score(test_predict, y_test)
print("在测试集上f1_mean的值为%.4f" % test_f1, end = ' ')
print("在测试集上的精确率的值为%.4f" % test_pre, end=' ')
print("在测试集上的查全率的值为%.4f" % test_rec)
end = time.time()
print("使用逻辑回归共耗时：%.2f秒"%(end-start))
#=================================================================
print("随机森林效果如下" + "=" * 30)
...
train_rec = metrics.recall_score(train_predict, y_train)
print("在训练集上f1_mean的值为%.4f" % train_f1, end=' ')
print("在训练集上的精确率的值为%.4f" % train_pre, end=' ')
print("在训练集上的查全率的值为%.4f" % train_rec)
